...
Running the identical stress calibration on both curves (3% underlying depeg, 1.5% discount widening, 8% band drop, representative LTV 0.80, two-day horizon, maturity haircuts 0.05 and 0.15 respectively), with the wrong-way factor ρ as the sensitivity axis:
...
Three structural readings survive any ρ choice. First, both ceilings land below pool TVL once stress contraction applies: at rho of 0.5 the central ceilings are 48% of pool TVL on PT-sUSDe and 57% on PT-reUSD, falling to 29% and 34% at rho of 0.3. Displayed depth is not usable depth. Second, the near-maturity anchor's ceiling is governed by the proportion cap times ρ; the far anchor's by the slippage curve times ρ: same formula, different binding term, which is why a single ported parameter set cannot serve both. Third, and this is the result that most constrains how the constraint should be read: it does not bind everywhere. Because stressed depth is linear in rho, the coverage ratio at the depth-agnostic benchmark ceiling is linear too, which locates the crossing exactly. On the near-maturity anchor the coverage constraint binds at any rho at or below 0.505, so across the whole sensitivity band. On the far-maturity anchor it binds only below rho of approximately 0.413: at rho of 0.5 the coverage ratio at the benchmark ceiling is 1.21, meaning the constraint is looser than the benchmark and the benchmark is the binding rule. The wrong-way factor is therefore not a presentational parameter. It decides whether an execution constraint binds at all on longer maturities, which is the direct argument for calibrating it per underlying family rather than assuming it.
...
- ρ calibration requires a sufficiently long joint history of underlying deviation and PT depth, which young pools may lack; until estimated per family, it is a published sensitivity input, and D* should be read as an indicative band, not a point. The estimation protocol is implemented in the repository (`estimate_rho.py`, fed by `fetch_rho_inputs.py` and the three Dune queries: underlying price history, pool TVL history, Pendle market liquidity context): joint history of underlying deviation and pool depth on long-history pools, transferred by underlying family to younger pools. No valid retained joint series is used to estimate rho in this release. In particular, the yield-accreting sUSDe share price is not treated as a USDe peg series. Rho therefore remains a disclosed forward stress input, presented as the [0.3, 0.5] sensitivity band until a matched unstaked-USDe and executable-depth history is retained.
- The framework assumes LLAMMA band parameters are known and stable per market; v2 design changes would shift V_liq. A band-aware V_liq fed by the live market's onchain position distribution is the natural refinement once LlamaLend v2 PT markets are live.
